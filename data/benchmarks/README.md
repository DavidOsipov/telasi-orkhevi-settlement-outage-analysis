...
`scripts/fetch_wbes_tbilisi.py` fails closed if an expected value is no longer returned as a JSON string, rather than silently losing lexical decimal precision through a binary floating-point parse.

## “Typical month” semantics

Do not interpret WBES `0.8 outages in a typical month` as `0.8 outages per arithmetic mean calendar month`.

The Enterprise Survey question asks how many outages an establishment experienced in a **typical month** during the last fiscal year. World Bank questionnaire guidance treats the typical month as the most common type of month regarding the characteristic being asked. It is therefore not a fixed-duration rate unit that can be exactly converted into “one outage every N days.”

Consequences for this repository:

- do not convert WBES `0.8` into a day interval;
- do not report the conditional SITE_B/WBES arithmetic quotient as an outage-rate ratio;
- do not state that Orkhevi has `X×` or `Y%` more outages than Tbilisi on the basis of these two non-identical measures;
- use WBES as independent Tbilisi context alongside the resident series, with the source/period/population differences made explicit.
