...
def render():
 ...
 n25,n26=ay(2025),ay(2026);ratio=Fraction(n26,n25);rel=ratio-1
 E('Same-source descriptive comparison (SITE_A / neighbor archive; same Orkhevi building; Jan 1-Aug 6):');E(f'  2025: {n25} emergency ETA-date groups');E(f'  2026: {n26} emergency ETA-date groups');E(f'  exact descriptive ratio: {display(ratio)}');E(f'  exact relative change: {display(rel,6)}');E(f'  exact relative change percent: {display(rel*100,6)}%');E('  This is an Orkhevi-building resident notification series, not a complete building-wide physical-outage rate.');E('')
