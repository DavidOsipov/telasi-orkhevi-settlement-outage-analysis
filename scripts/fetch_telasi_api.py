...
def orkhevi_match_kind(text: str) -> str:
    ...
    Search results for ``ორხევი`` include the settlement, an industrial zone,
    exits/streets, and parenthetical locality qualifiers. Only the first class
    is treated as an explicit settlement-name match.
    """
...
def make_search_payload(search_text: str) -> dict:
    # This is the payload emitted by the public site's search interaction.
...
def make_list_payload(page_number: int, per_page: int, selected_lang: str) -> dict:
    # This is the separate paginated-list payload emitted by the public page.
    return {
        "searchText": "",
...
def fetch(payload: dict, selected_lang: str, timeout: float) -> tuple[bytes, int, str]:
    # ensure_ascii=False is intentional: the public browser request sends the
    # Georgian query as UTF-8 JSON. Avoid shell/cURL round-trips that can mangle
    # non-ASCII clipboard text on some Windows import paths.
...
def normalize_rows(document: dict) -> list[dict]:
    ...
    for item in content["list"]:
        ...
        searchable_text = " ".join(
            filter(
                None,
                [html_to_text(item.get("title")), html_to_text(item.get("teaser")), editor_text],
            )
        )
        rows.append(
            {
...
def main() -> int:
    ...
    parser.add_argument("--list-mode", action="store_true", help="Use the page's taxonomy/pagination payload instead of search mode")
    parser.add_argument("--page-number", type=int, default=1)
    parser.add_argument("--per-page", type=int, default=100)
    parser.add_argument("--selected-lang", default="ka")
    parser.add_argument("--input-json", help="Normalize an already captured response instead of making a network request")
    ...
    for row in rows:
        counts[row["publication_class"]] = counts.get(row["publication_class"], 0) + 1
    ...
    metadata = {
        ...
        "publication_class_counts": counts,
